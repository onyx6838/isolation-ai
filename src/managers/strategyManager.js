...
const StrategyManager = {
  ...
  minimax: function (tree, playerIndex, players, values, width, height) {
    let bestState = null; // state back to comp
    let bestVal = -9999;  // last val alpha
    let beta = 9999;      // β

    console.log('Using AI strategy minimax.');

    const getSuccessors = node => { // adjacent
      return node ? node.children : [];
    };

    const isTerminal = node => {  // last node - true if have no children, false if have children 
      return node ? node.children.length === 0 : true;
    };

    const getUtility = node => {  // get value
    ...
    // procedure of alpha_beta()
    const successors = getSuccessors(tree); // get tree node
    successors.forEach(state => {
